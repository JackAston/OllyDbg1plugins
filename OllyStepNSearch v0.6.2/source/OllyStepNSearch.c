...
//	25/11/2010 version 0.6.2	Added unicode/pascal search and t_reg->modified optimization by Mark D. Adams http://myblog.org
...
#define INI_SEARCHUNICODE "SearchUnicode"
#define INI_SEARCHPASCAL "SearchPascal"

#define URL "https://DidierStevens.com"
#define URL2 "http://myblog.org"
...
int		iFlagSearchUnicode;		// Flag, true when we want to do unicode search
int		iFlagSearchPascal;		// Flag, true when we want to do pascal search
...
void LoadFromOllyDbgIni(void)
{
	...
	iFlagSearchUnicode = Pluginreadintfromini(hiDLL, INI_SEARCHUNICODE, 0);
	iFlagSearchPascal = Pluginreadintfromini(hiDLL, INI_SEARCHPASCAL, 0);	
	...
	Addtolist(0, 0, "OllyStepNSearch iFlagSearchUnicode = %d", iFlagSearchUnicode);
	Addtolist(0, 0, "OllyStepNSearch iFlagSearchPascal = %d", iFlagSearchPascal);	
}
...
void SaveToOllyDbgIni(void)
{
	...
	Pluginwriteinttoini(hiDLL, INI_SEARCHUNICODE, iFlagSearchUnicode);
	Pluginwriteinttoini(hiDLL, INI_SEARCHPASCAL, iFlagSearchPascal);
}
...
extc int _export cdecl ODBG_Plugininit(int ollydbgversion,HWND hw,ulong *features)
{
	...
	{
		Addtolist(0, 1,  "OllyStepNSearch not compiled for this OllyDbg version");
		return -1;
	}
	...
	Addtolist(0, -1, "  Public domain by Didier Stevens %s and Mark D. Adams %s", URL, URL2);
...
int ODBG_Pausedex(int reason, int extdata, t_reg *reg, DEBUG_EVENT *debugevent)
{
	...
	if (reg == NULL)
	{
		Addtolist(0, 1,  "Error getting registers, pausing");
		return 0;
	}
	...
	iFound = 0;
	if (iStep == STEP_IN && reg->modified != 0)
		for (iIter = 0; iIter < 8; iIter++)
		{
			if (ulRegPrev[iIter] != reg->r[iIter])
			{
				if (Decodeascii(reg->r[iIter], achText, TEXTLEN, DASC_NOHEX) != 0)
				{
					if (iFlagSearchForSearchText && iFlagSearchInRegisters && strstr(achText, achSearchText) != NULL)
					{
						Addtolist(reg->ip, 0, "OllyStepNSearch: %s %08lX ASCII %s", aszRegName[iIter], reg->r[iIter], achText);
						iFound = 1;
					}
				}
				if (iFlagSearchUnicode && Decodeascii(reg->r[iIter], achText, TEXTLEN, DASC_PASCAL) != 0)
				{
					if (iFlagSearchForSearchText && iFlagSearchInRegisters && strstr(achText, achSearchText) != NULL)
					{
						Addtolist(reg->ip, 0, "OllyStepNSearch: %s %08lX pascal %s", aszRegName[iIter], reg->r[iIter], achText);
						iFound = 1;
					}
				}
				if (iFlagSearchPascal && Decodeunicode(reg->r[iIter], achText, TEXTLEN) != 0)
				{
					if (iFlagSearchForSearchText && iFlagSearchInRegisters && strstr(achText, achSearchText) != NULL)
					{
						Addtolist(reg->ip, 0, "OllyStepNSearch: %s %08lX unicode %s", aszRegName[iIter], reg->r[iIter], achText);
						iFound = 1;
					}
			   	}
...
LRESULT CALLBACK OptDlgProc(HWND hwDialog, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	switch (msg) 
	{
		case WM_INITDIALOG:
			...
			SendMessage(GetDlgItem(hwDialog, IDC_CHK_UNICODE), BM_SETCHECK, (WPARAM)iFlagSearchUnicode, 0L);
			SendMessage(GetDlgItem(hwDialog, IDC_CHK_PASCAL), BM_SETCHECK, (WPARAM)iFlagSearchPascal, 0L);
			break;
		...
		case WM_COMMAND:
			switch (LOWORD(wp))
			{
				case IDOK:
					...
					iFlagSearchUnicode = ((IsDlgButtonChecked(hwDialog, IDC_CHK_UNICODE) == BST_CHECKED) ? 1 : 0);
					iFlagSearchPascal = ((IsDlgButtonChecked(hwDialog, IDC_CHK_PASCAL) == BST_CHECKED) ? 1 : 0);
...
LRESULT CALLBACK HlpDlgProc(HWND hwDialog, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	switch (msg) 
	{
		case WM_INITDIALOG:
			...
			{
				sprintf(pHelp, szHelp, VERSION, URL, URL2);
