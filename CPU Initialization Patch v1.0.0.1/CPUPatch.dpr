library CPUPatch;

uses
  Windows,
  SysUtils,
  Plugin,
  IniFiles;

const
  WAIT_TIME_OFFSET = $00439616 + 1;  

resourcestring
  VERSION = '1.0.0.1';
  AUTHOR  = 'BlurCode';
  PLUGIN_NAME = 'CPU Initialization Patch';
  ABOUT   = 'Used:' + #13#10 +
            'OllyDbg Plugin SDK translation by TQN' + #13#10 +
            'Delphi 11';
  ININAME = 'cpupatch.ini';

var
  g_hwndOlly: HWND;
  g_hmodOlly: HMODULE;
  SaveDLLProc: TDLLProc;

function ODBG_Plugindata(name: PChar): Integer; cdecl;
begin
  StrLCopy(name, PChar(PLUGIN_NAME), 32); // Name of plugin
  Result := PLUGIN_VERSION;
end;

function ODBG_Plugininit(ollydbgversion: Integer; hWndOlly: HWND; features: PULONG): Integer; cdecl;
begin
  if (ollydbgversion < PLUGIN_VERSION) then begin
    Result := -1;
    Exit;
  end;

  g_hwndOlly := hWndOlly;
  Addtolist(0,  0, 'CPU Initialization Patch');
  Addtolist(0, -1, ' version %s by %s', VERSION, AUTHOR);  

  Result := 0;
end;

function ODBG_Pluginmenu(origin: Integer; pData: PChar; pItem: Pointer): Integer; cdecl;
begin
  case origin of
    PM_MAIN:
      begin
        // Plugin menu in main window
        StrCopy(pData, '0 &Enable, 1 &Disable|3 &About...');
        Result := 1;
      end;
  else
    Result := 0; // Any other window
  end;
end;

procedure ODBG_Pluginaction(origin: Integer; action: Integer; pItem: Pointer); cdecl;

  procedure Patch(const Value: Byte);
  var
    OldProtect, tempOldProtect: Cardinal;
  begin
      if VirtualProtect(Pointer(WAIT_TIME_OFFSET), 1, PAGE_EXECUTE_READWRITE, OldProtect) then begin
        //New method
        PByte(Pointer(WAIT_TIME_OFFSET))^ := Value;

        //Restore original Protect
        VirtualProtect(Pointer(WAIT_TIME_OFFSET), 1, OldProtect, tempOldProtect);
      end else
        MessageBoxA(0, 'Failed to apply patch :(', 'Error', MB_ICONERROR);
  end;

var
  IniFile: TFileName;

  fIni: TIniFile;
begin
  IniFile:= ExtractFilePath(ParamStr(0)) + ININAME;

  fIni:= TIniFile.Create(IniFile);
  try
    case origin of
     PM_MAIN:
      begin
        case action of
          0:
           begin
             Patch(10);

             fIni.WriteBool('Options', 'Enabled', True);
           end;
          1:
           begin
             Patch( 0);

             fIni.WriteBool('Options', 'Enabled', False);
           end;
          3: MessageBox(g_hwndOlly, PChar(ABOUT), PChar(PLUGIN_NAME), MB_OK);
        end;
      end;
    end;
  finally
    fIni.Free;
  end;
end;

function ODBG_Paused(reason: Integer; reg: p_reg): Integer; cdecl;
begin
  ODBG_Paused:= 0;
end;

procedure DLLExit(dwReason: DWORD);
begin
  if (dwReason = DLL_PROCESS_DETACH) then begin
    // Uninitialize code here
    //OutputDebugString('Plugin unloaded by DLL_PROCESS_DETACH');
  end;

  // Call saved entry point procedure
  if Assigned(SaveDLLProc) then
    SaveDLLProc(dwReason);
end;

exports
  ODBG_Plugindata   name '_ODBG_Plugindata',
  ODBG_Plugininit   name '_ODBG_Plugininit',
  ODBG_Pluginmenu   name '_ODBG_Pluginmenu',
  ODBG_Pluginaction name '_ODBG_Pluginaction',
  ODBG_Paused       name '_ODBG_Paused';

procedure ReadOptions;
var
  IniFile: TFileName;
  
  fIni: TIniFile;
begin
  IniFile:= ExtractFilePath(ParamStr(0)) + ININAME;

  fIni:= TIniFile.Create(IniFile);
  try
    if fIni.ReadBool('Options', 'Enabled', False) then
      ODBG_Pluginaction(PM_MAIN, 0, nil);
  finally
    fIni.Free;
  end;
end;  

begin
  // Initialize code here
  SaveDLLProc := @DLLProc;
  DLLProc := @DLLExit;

  g_hmodOlly := GetModuleHandle(nil);

  ReadOptions;
end.
