...
t_wndprog_data *ppl;
	...
	switch (msg) 
	{
		...
		case WM_USER_MENU:
			...
			AppendMenu(menu,MF_POPUP,(DWORD) mRun,"运行脚本"); 
            ...
            AppendMenu(menu,MF_POPUP,(DWORD) mCmd,"常用脱壳和破解方法");
			AppendMenu(mCmd,MF_MENUBREAK,50,"请选择下列方法");
			...
			if (menu!=NULL && ppl!=NULL) 
			{
				...
				AppendMenu(menu,MF_STRING, 36,"在光标处下断点并执行\tF4");
 				AppendMenu(menu,MF_STRING, 33,"单步\tS");
			...
			if (ollylang->variables.size() > 0) 
			{
				...
				AppendMenu(menu,MF_SEPARATOR,0,"-");
				AppendMenu(menu,MF_POPUP,(DWORD) mVars,"编辑变量"); 
...
void initProgTable() {
  	...
  	if (ollylang->wndProg.bar.nbar==0) 
	{
		...
		ollylang->wndProg.bar.name[4]="计算结果 <---";
		ollylang->wndProg.bar.defdx[4]=20;
		ollylang->wndProg.bar.mode[4]=BAR_NOSORT;

		ollylang->wndProg.bar.name[5]="注释";
	...
	Quicktablewindow(&ollylang->wndProg,15,5,wndprogclass,"脚本运行窗口");
