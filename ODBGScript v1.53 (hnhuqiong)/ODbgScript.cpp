...
extc int _export cdecl ODBG_Plugininit(int odbgversion, HWND hw, ulong *features) 
{
	...
	Addtolist(0, -1,"  by hnhuqiong@126.com from ODbgScript 1.47 by Epsylon3");
	Addtolist(0,-1,"  Compiled %s %s   www.pediy.com",__DATE__, __TIME__);
...
{	
	...
	if(status == STAT_STOPPED && (script_state == SS_RUNNING || script_state == SS_LOADED))
	{
		...
		{
			delete ollylang;
			MessageBox(hwndOllyDbg(), "插件出现问题!\n请联系hnhuqiong@126.com.", "ODbgScript", MB_OK | MB_ICONERROR | MB_TOPMOST);
		}
...
//		",12 TEST2"
      ...
      if (pd==NULL || pd->size==0)
		...
		ZeroMemory(buff, sizeof(buff));
		strcpy(buff, "# 30 脚本运行窗口(&W)...\t,运行脚本(&P){0 打开...|");
		mruGetCurrentMenu(&buff[strlen(buff)]);
 		strcpy(&buff[strlen(buff)],"}  ,脚本功能{|4 单步 ,2 继续 } "); 	
		strcpy(data,buff);			
		return 1;
	}
...
extc void _export cdecl ODBG_Pluginaction(int origin, int action, void *item) 
{
  ...
  switch (action) 
  {
	case 0: // Run script
		...
		ofn.lpstrFilter = "ODbgScripts\0*.osc;*.txt\0All\0*.*\0";
		ofn.nFilterIndex = 1;
	...
	case 1: // Abort
		ollylang->Reset(); 
		MessageBox(hwmain,"脚本中止!","ODbgScript",MB_OK|MB_ICONEXCLAMATION);
		ollylang->Pause();
		break;
    ...
    case 10:
		sprintf(s,"ODbgScript plugin v%i.%i\n"
			      "by hnuqiong@126.com\n\n\n"
				  "From ODbgScript 1.47 written by Epsylon3\n"
