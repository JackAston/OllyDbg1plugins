...
;Changed due to problems with export table elimination by OllyAdvanced etc

extern __imp__Guardmemory
mov eax, [__imp__Guardmemory]
...
;IMPORTANT BUGFIX
;mov edx, [edx+3*4]         ;EXCEPTION_DEBUG_INFO._EXCEPTION_RECORD.ExceptionAddress <-- WRONG: this is EIP

mov edx, [edx+5*4+1*4]      ;EXCEPTION_DEBUG_INFO._EXCEPTION_RECORD.ExceptionInformation[1]
                            ;MSDN: The second array element specifies the virtual address of the inaccessible data.
...
xor ecx, ecx        ;it will be tested for some more exceptions, make sure it doesn't pass those
...
text:
db "OllyGuard v0.1.2 by MOID / TSRh", 0xD, 0xA
